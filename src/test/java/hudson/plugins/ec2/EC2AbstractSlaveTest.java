...
public class EC2AbstractSlaveTest {
    ...
    public void testMaxUsesBackwardCompat() throws Exception {
        ...
        templates.add(orig);
        AmazonEC2Cloud ac = new AmazonEC2Cloud("eu-west-1", false, "abc", "eu-west-1", "ghi", "3", templates, null, null);
        r.jenkins.clouds.add(ac);
        EC2AbstractSlave slave = new EC2AbstractSlave("name", "", description, "fs", 1, null, "label", null, null, "init", "tmpDir", new ArrayList<NodeProperty<?>>(), "root", "jvm", false, "idle", null, "ec2-eu-west-1", false, Integer.MAX_VALUE, new UnixData("remote", null, null, "22", null), ConnectionStrategy.PRIVATE_IP, 0)  {
            @Override
            public void terminate() {
            }
